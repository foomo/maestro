...
hero:
  name: maestro
  text: Atomic in-memory state replication for Go
  tagline: One writer, N readers, over NATS. Every reader flips to the new version together, or keeps the old one — partial updates are never observable.
  image:
    src: /logo.png
    alt: maestro
  actions:
